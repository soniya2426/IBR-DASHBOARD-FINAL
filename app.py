...
if up1 is not None and up2 is not None:
    ...
    estee = pd.read_excel("data/estee_lauder_dataset_50.xlsx", engine="openpyxl")
    shiffa = pd.read_excel("data/shiffa_synthetic_dataset_50.xlsx", engine="openpyxl")
...
with tab1:
    ...
    if brand_pick == "Estee Lauder":
        ...
        data = df
    for v in pick:
        fig = px.histogram(data, x=v, color="brand" if brand_pick=="Both" else None,
...
with tab2:
    ...
    if not num.empty:
        corr = num.corr()
        fig = px.imshow(corr, text_auto=True, aspect="auto", title=f"Correlation ({scope})",
...
with tab5:
    ...
    with c1:
        ...
        if len(attrs)>=2:
            ...
            fig = px.scatter(pcadf, x="PC1", y="PC2", color="brand", opacity=0.3,
                             title="Respondent-level PCA (2D)")
            fig.add_scatter(x=centroids["PC1"], y=centroids["PC2"], mode="markers+text",
                            text=centroids["brand"], marker=dict(size=18, line=dict(width=2, color="black")),
    ...
    with c2:
        ...
        seg_vars = st.multiselect("Variables for clustering",
                                  [v for v in ["personalisation","brand_authenticity","halal_certified","pricing",
                                               "brand_loyalty","willingness_to_buy","transparency",
                                               "consistency","sustainable"] if v in df.columns],
